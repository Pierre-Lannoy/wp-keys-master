...
if ( ! class_exists( 'PerfOpsOne\AdminMenus' ) ) {
	class AdminMenus {
		...
		public static function initialize() {
			foreach ( apply_filters( 'init_perfops_admin_menus', [] ) as $menu => $submenus ) {
				if ( ! in_array( 'perfopsone-' . $menu, self::$slugs, true ) ) {
					switch ( $menu ) {
						case 'analytics':
							add_menu_page( esc_html__( 'Dashboard', 'keys-master' ), sprintf( esc_html__( '%s Analytics', 'keys-master' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_analytics_page' ], 'dashicons-chart-bar', 210874 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Dashboard', 'keys-master' ), __( 'Dashboard', 'keys-master' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_analytics_page' ], 0 );
							break;
						case 'tools':
							add_menu_page( esc_html__( 'Available Tools', 'keys-master' ), sprintf( esc_html__( '%s Tools', 'keys-master' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_tools_page' ], 'dashicons-admin-tools', 210876 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Tools', 'keys-master' ), __( 'Available Tools', 'keys-master' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_tools_page' ], 0 );
							break;
						case 'insights':
							add_menu_page( esc_html__( 'Available Reports', 'keys-master' ), sprintf( esc_html__( '%s Insights', 'keys-master' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_insights_page' ], 'dashicons-lightbulb', 210875 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Reports', 'keys-master' ), __( 'Available Reports', 'keys-master' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_insights_page' ], 0 );
							break;
						case 'records':
							add_menu_page( esc_html__( 'Available Catalogues', 'keys-master' ), sprintf( esc_html__( '%s Records', 'keys-master' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_records_page' ], 'dashicons-book', 210873 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Catalogues', 'keys-master' ), __( 'Available Catalogues', 'keys-master' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_records_page' ], 0 );
							break;
						case 'settings':
							add_menu_page( esc_html__( 'Control Center', 'keys-master' ), sprintf( esc_html__( '%s Settings', 'keys-master' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_settings_page' ], 'dashicons-admin-settings', 210871 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Control Center', 'keys-master' ), __( 'Control Center', 'keys-master' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_settings_page' ], 0 );
							break;
						case 'consoles':
							add_menu_page( esc_html__( 'Available Consoles', 'keys-master' ), sprintf( esc_html__( '%s Consoles', 'keys-master' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_consoles_page' ], 'dashicons-embed-generic', 210872 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Consoles', 'keys-master' ), __( 'Available Consoles', 'keys-master' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_consoles_page' ], 0 );
							break;
					}
		...
		private static function display_as_lines( $items ) {
			...
			foreach ( $items as $item ) {
				$links   = [];
				$links[] = '<a href="' . $item['url'] . '"/>' . __( 'settings', 'keys-master' ) . '</a>';
				$links[] = '<a href="' . $item['url'] . '&tab=about"/>' . __( 'about', 'keys-master' ) . '</a>';
				$links[] = '<a href="https://wordpress.org/support/plugin/' . $item['slug'] . '" target="_blank">' . __( 'support', 'keys-master' ) . '</a>';
				$links[] = '<a href="https://github.com/Pierre-Lannoy/wp-' . $item['slug'] . '" target="_blank">' . __( 'contribution', 'keys-master' ) . '</a>';
				$disp   .= '<div class="po-container">';
				$disp   .= ' <div class="po-actionable">';
