...
class Date {
	...
	public static function get_positive_time_diff_from_mysql_utc( $from ) {
		if ( strtotime( $from ) < time() ) {
			return sprintf( esc_html__( '%s ago', 'keys-master' ), human_time_diff( strtotime( $from ) ) );
		} else {
			return esc_html__( 'currently', 'keys-master' );
		}
	}
	...
	public static function get_time_diff_from_mysql_utc( $from ) {
		if ( strtotime( $from ) < time() ) {
			...
			return sprintf( esc_html__( 'in %s', 'keys-master' ), human_time_diff( strtotime( $from ) ) );
		}
	}
	...
	public static function get_age_array_from_seconds( $age, $legend = false, $abbrev = false ) {
		...
		if ( $abbrev ) {
			$intervals = [
				[ 60, _x( 'sec', 'Unit abbreviation - Stands for "second".', 'keys-master' ), _x( 'sec', 'Unit abbreviation - Stands for "second".', 'keys-master' ) ],
				[ 60, _x( 'min', 'Unit abbreviation - Stands for "minute".', 'keys-master' ), _x( 'min', 'Unit abbreviation - Stands for "minute".', 'keys-master' ) ],
				[ 100000, _x( 'hr', 'Unit abbreviation - Stands for "hour".', 'keys-master' ), _x( 'hr', 'Unit abbreviation - Stands for "hour".', 'keys-master' ) ],
			];
		} else {
			$intervals = [
				[ 60, _n( 'second', 'seconds', 1, 'keys-master' ), _n( 'second', 'seconds', 2, 'keys-master' ) ],
				[ 60, _n( 'minute', 'minutes', 1, 'keys-master' ), _n( 'minute', 'minutes', 2, 'keys-master' ) ],
				[ 100000, _n( 'hour', 'hours', 1, 'keys-master' ), _n( 'hour', 'hours', 2, 'keys-master' ) ],
			];
		}
		$value = [];
