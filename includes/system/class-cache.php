...
class Cache {
	 ...
	 */
	private static $pool_name = POKM_SLUG;
	...
	public static function init() {
		...
		add_action( 'shutdown', [ 'KeysMaster\System\Cache', 'log_debug' ], 10, 0 );
		add_filter( 'perfopsone_icache_introspection', [ 'KeysMaster\System\Cache', 'introspection' ] );
	}
	...
	public static function introspection( $endpoints ) {
		$endpoints[ POKM_SLUG ] = [ 'name' => POKM_PRODUCT_NAME, 'version' => POKM_VERSION, 'endpoint' => [ 'KeysMaster\System\Cache', 'get_analytics' ] ];
		return $endpoints;
	}
		...
		if ( self::$apcu_available ) {
			if ( function_exists( 'apcu_cache_info' ) && function_exists( 'apcu_delete' ) ) {
				try {
					...
					\DecaLog\Engine::eventsLogger( POKM_SLUG )->error( sprintf( 'Unable to query APCu status: %s.', $e->getMessage() ), [ 'code' => $e->getCode() ] );
				}
			}
		} else {
	...
	public static function log_debug() {
		...
		if ( 0 !== (int) $analytics['hit']['count'] || 0 !== (int) $analytics['miss']['count'] ) {
			\DecaLog\Engine::eventsLogger( POKM_SLUG )->debug( $log );
		}
	}
