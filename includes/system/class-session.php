			...
			if ( array_key_exists( $role, $settings ) ) {
				...
				if ( '' === $mode || 0 === $limit ) {
					...
					if ( 'allow' === $result ) {
						...
						$this->die( __( '<strong>FORBIDDEN</strong>: ', 'keys-master' ) . apply_filters( 'sessions_bad_ip_message', __( 'You\'re not allowed to initiate a new session from your current IP address.', 'keys-master' ) ), 403 );
					}
					if ( 'allow' !== $result ) {
						...
						switch ( $method ) {
							...
							case 'default':
								...
								return new \WP_Error( '403', __( '<strong>ERROR</strong>: ', 'keys-master' ) . apply_filters( 'sessions_blocked_message', __( 'You\'re not allowed to initiate a new session because your maximum number of active sessions has been reached.', 'keys-master' ) ) );
							default:
								Logger::warning( sprintf( 'New session not allowed for %s. Reason: %s.', User::get_user_string( $this->user_id ), str_replace( 'device-', ' ', $mode ) ), 403 );
								$this->die( __( '<strong>FORBIDDEN</strong>: ', 'keys-master' ) . apply_filters( 'sessions_blocked_message', __( 'You\'re not allowed to initiate a new session because your maximum number of active sessions has been reached.', 'keys-master' ) ), 403 );
						}
					} else {
						Logger::debug( sprintf( 'New session allowed for %s.', User::get_user_string( $this->user_id ) ), 200 );
					}
