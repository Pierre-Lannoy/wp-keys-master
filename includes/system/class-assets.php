...
class Assets {
	...
	public function prefetch() {
		if ( Option::network_get( 'use_cdn' ) && POKM_CDN_AVAILABLE ) {
	...
	public function register_style( $handle, $src, $file, $deps = [], $media = 'all' ) {
		if ( Option::network_get( 'use_cdn' ) && POKM_CDN_AVAILABLE ) {
			if ( POKM_ADMIN_URL === $src ) {
				$file = 'https://cdn.jsdelivr.net/wp/' . POKM_SLUG . '/tags/' . POKM_VERSION . '/admin/' . $file;
			} else {
				$file = 'https://cdn.jsdelivr.net/wp/' . POKM_SLUG . '/tags/' . POKM_VERSION . '/public/' . $file;
			}
			// phpcs:ignore
