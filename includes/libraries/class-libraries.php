...
class Libraries {
	...
	public static function init() {
		...
		self::$psr4_libraries['psr-03']      = [
			'name'    => 'PSR-3',
			'prefix'  => 'Psr\\Log',
			'base'    => POKM_VENDOR_DIR . 'psr/log-v' . $psrlog_version . '/',
		...
		self::$psr4_libraries['psr-07']      = [
			...
			'base'    => POKM_VENDOR_DIR . 'psr/http-message/',
			'version' => '2.0',
			'author'  => 'PHP Framework Interop Group',
			'url'     => 'https://www.php-fig.org/',
			'license' => 'mit',
		...
		self::$psr4_libraries['psr-18']      = [
			...
			'base'    => POKM_VENDOR_DIR . 'psr/http-client/',
			'version' => '1.0.3',
			'author'  => 'PHP Framework Interop Group',
			'url'     => 'https://www.php-fig.org/',
			'license' => 'mit',
		...
		self::$psr4_libraries['decalog-sdk'] = [
			'name'    => 'DecaLog SDK',
			'prefix'  => 'DecaLog',
			'base'    => POKM_VENDOR_DIR . 'decalog-sdk/',
			'version' => '4.2.0',
	...
	public function sc_get_list( $attributes ) {
		...
		$item['version'] = '3.0.5';
		$item['author']  = sprintf( esc_html__( '%s & contributors', 'keys-master' ), 'Dan Grossman' );
		...
		$item['name']    = 'Moment';
		$item['version'] = '2.29.4';
		$item['author']  = sprintf( esc_html__( '%s & contributors', 'keys-master' ), 'Tim Wood' );
		...
		$item['version'] = '0.11.4';
		$item['author']  = sprintf( esc_html__( '%s & contributors', 'keys-master' ), 'Gion Kunz' );
		...
		if ( 'html' === $style ) {
			...
			foreach ( $list as $library ) {
				...
				$items[] = sprintf( __( '<a href="%1$s">%2$s %3$s</a>%4$s by %5$s (%6$s)', 'keys-master' ), $library['url'], $library['name'], 'v' . $library['version'], $library['langs'], $library['author'], $library['license'] );
			}
